...
Perbaikan utama:
- Whitelist state diperluas agar widget key tidak terhapus saat re-render
- Prefix yang lebih luas untuk menangkap semua key widget Streamlit
"""
...
# Kunci session state yang selalu diizinkan (whitelist eksak)
...
# Prefix widget Streamlit yang diizinkan
# PENTING: Streamlit menyimpan state widget secara otomatis menggunakan key
# yang didaftarkan di parameter key=. Key-key ini HARUS dikecualikan agar
# widget tidak ter-reset setiap kali halaman di-render ulang.
_ALLOWED_STATE_PREFIXES: tuple[str, ...] = (
    # Widget yang didaftarkan secara eksplisit di app.py
    "forecast_",        # key="forecast_uploader" (file uploader)
    "model_",           # key=f"model_{product_name}" (selectbox per produk)
    "mape_",            # key="mape_product_select" (selectbox MAPE)
    "eda_",             # widget di halaman EDA
    "fc_",              # key="fc_periods" (slider forecast horizon)
    "btn_",             # key="btn_generate" (tombol generate)

    # Key internal Streamlit
    "FormSubmitter:",
    "$$WIDGET_ID",
    "_stcore_",
    "uploader_",

    # Catch-all untuk key Streamlit internal lainnya
    "file_uploader",
    "selectbox",
    "slider",
    "button",
...
def enforce_state_whitelist() -> None:
    ...
    Key widget Streamlit (file_uploader, selectbox, slider, dll.)
    DIKECUALIKAN dari penghapusan agar widget tidak ter-reset.

    FIX: Sebelumnya whitelist terlalu ketat sehingga key widget dihapus,
    menyebabkan file uploader / selectbox ter-reset setiap render dan
    proses forecast tidak bisa berjalan.
    """
    rogue_keys = [
        k for k in list(st.session_state.keys())
        if k not in _ALLOWED_STATE_KEYS
        and not any(k.startswith(prefix) for prefix in _ALLOWED_STATE_PREFIXES)
        # Izinkan semua key yang berisi karakter UUID atau hash Streamlit
...
def _looks_like_internal_key(key: str) -> bool:
    """
    Deteksi apakah key adalah key internal Streamlit
    (biasanya berisi UUID, hash hex, atau karakter khusus).
    """
    # Key dengan karakter $ atau non-ASCII biasanya internal Streamlit
    if any(c in key for c in ("$", "\x00", "widget")):
        return True
    # Key yang sangat panjang (>80 karakter) biasanya hash internal
    if len(key) > 80:
        return True
    return False
...
ALLOWED_EXTENSIONS: set[str] = {".csv"}  # Hanya CSV sesuai permintaan
...
def validate_upload(uploaded_file) -> tuple[bool, str]:
    ...
    if ext not in ALLOWED_EXTENSIONS:
        ...
        return False, (
            f"Tipe file '{ext}' tidak diizinkan. Gunakan file CSV (.csv)."
        )
...
def validate_dataframe(df: pd.DataFrame) -> tuple[bool, str]:
    """Validasi DataFrame hasil parsing."""
    if df.empty:
...
def audit_log(action: str, detail: str = "") -> None:
    """Catat aksi ke audit log (hanya metadata, bukan isi data)."""
    sid = st.session_state.get("session_id", "unknown")
