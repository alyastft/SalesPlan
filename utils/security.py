...
# Kunci session state yang diizinkan disimpan (whitelist eksak)
...
# ── FIX: Prefix key widget Streamlit yang diizinkan ──────────────────────────
# Streamlit menyimpan state widget secara otomatis dengan key yang didaftarkan
# di parameter `key=`. Key-key ini harus dikecualikan dari penghapusan agar
# widget (file uploader, selectbox, slider, dll.) tidak ter-reset setiap render.
_ALLOWED_STATE_PREFIXES: tuple[str, ...] = (
    "forecast",         # st.file_uploader(key="forecast")
    "model_",           # st.selectbox(key=f"model_{product_name}")
    "mape_",            # widget di halaman MAPE, mis. key="mape_product_select"
    "eda_",             # widget di halaman Data Analysis / EDA
    "FormSubmitter:",   # key internal form Streamlit
    "$$WIDGET_ID",      # key internal Streamlit lainnya
)
...
def enforce_state_whitelist() -> None:
    """
    Hapus kunci session state yang tidak ada di whitelist eksak
    maupun tidak diawali dengan prefix widget yang diizinkan.

    Penting: Streamlit menyimpan state widget (file_uploader, selectbox, dll.)
    secara otomatis menggunakan key yang didaftarkan. Key-key ini HARUS
    dikecualikan agar widget tidak ter-reset setiap kali halaman di-render ulang.
    """
    rogue_keys = [
        k for k in list(st.session_state.keys())
    ...
    if rogue_keys:
        log.warning(
            "Kunci state tidak diizinkan ditemukan dan dihapus: %s", rogue_keys
        )
        for k in rogue_keys:
            del st.session_state[k]
...
def validate_dataframe(df: pd.DataFrame) -> tuple[bool, str]:
    ...
    if len(df) > MAX_ROWS:
        return False, (
            f"Data terlalu besar ({len(df):,} baris). "
            f"Batas: {MAX_ROWS:,} baris."
        )

    if len(df.columns) > MAX_COLS:
        return False, (
            f"Terlalu banyak kolom ({len(df.columns)}). "
            f"Batas: {MAX_COLS}."
        )
